<?php foreach ($nodes->order_by('position', 'ASC')->find_all() as $node): ?>
	...
	<li<?=HTML::attributes(['class' => 'list-group-item'.($children ? ' has-children' : '').(isset($open[$node->id]) ? ' open' : '').($node->visible ? '' : ' disabled').($node->published !== $node->revision ? ' unpublished' : NULL)]);?>>
