...
class Auth_Prime extends Auth {
	...
	public function logged_in($role = NULL)
	{
		...
		if ($user instanceof Model_Prime_User AND $user->loaded())
		{
			...
			return $user->has('prime_roles', $roles);
		}
	}
	...
	public function auto_login()
	{
		if ($token = Cookie::get('authautologin'))
		{
			...
			if ($token->loaded() AND $token->prime_user->loaded())
			{
				if ($token->user_agent === sha1(Request::$user_agent))
				{
					...
					$this->complete_login($token->prime_user);

					// Automatic login was successful
					return $token->prime_user;
				}
	...
	public function logout($destroy = FALSE, $logout_all = FALSE)
	{
		...
		if ($token = Cookie::get('authautologin'))
		{
			...
			if ($token->loaded() AND $logout_all)
			{
				...
				$tokens = ORM::factory('Prime_User_Token')->where('prime_user_id','=',$token->prime_user_id)->find_all();
