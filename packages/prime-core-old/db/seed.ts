...
export const seed = async () => {
  ...
  const seoType = await ContentType.create({
    title: 'SEO',
    userId: user.id,
    isTemplate: true,
    groups: ['SEO'],
  ...
  const blogType = await ContentType.create({
    title: 'Blog',
    userId: user.id,
    settings: { contentTypeIds: [seoType.id] },
  });
  const blogFields = [
    await ContentTypeField.create({
      title: 'Title',
  ...
  await acl.allow('sample-editor', `ContentType.${blogType.id}`, [
    'create',
    'read',
    'update',
    'delete',
  ...
  await slice1.$add('fields', [
    await ContentTypeField.create({ title: 'Foo', name: 'foo', type: 'string' }),
  ]);
  // next slice type
  ...
  await slice2.$add('fields', [
    await ContentTypeField.create({ title: 'Bar', name: 'bar', type: 'string' }),
  ]);
  authorType.$add('fields', [
    await ContentTypeField.create({
      ...
      name: 'body',
      type: 'slice',
      options: { contentTypeIds: [slice1.id, slice2.id] },
    }),
  ]);
