...
import {
  ...
  ContentTypeFieldGroupInputType,
  getFields,
  setFields,
} from './processFields';
  ...
  const allContentEntries = {
    ...
    resolve: relay.createConnectionResolver({
      ...
      before: async (findOptions, args, context) => {
        ...
        findOptions.order = [
          sequelize.literal(
            `CASE WHEN "language" = ${sequelize.escape(args.language)} THEN 1 ELSE 2 END`
          ),
          [sort, order],
        ];
          ...
          values.edges.map(async edge => {
            ...
            if (contentTypeDisplay.has(node.contentTypeId)) {
              const displayFieldValue = get(
                node.data,
                contentTypeDisplay.get(node.contentTypeId),
                ''
              );
              ...
              node.display = get(
                node.data,
                'title',
                get(node.data, 'name', get(node.data, dataKeys[0], node.entryId))
              );
            }
    ...
    publishContentRelease: {
      ...
      async resolve(root, args, context, info) {
        ...
        await ContentEntry.update(
          ...
          { where: { contentReleaseId: args.id } }
        );
        const contentRelease = await ContentRelease.findOne({ where: { id: args.id } });
        if (contentRelease) {
    ...
    updateContentEntry: {
      ...
      async resolve(root, args, context, info) {
        ...
        if (entry) {
          ...
          updatedEntry.data = await entryTransformer.transformOutput(
            updatedEntry.data,
            entry.contentTypeId
          );
