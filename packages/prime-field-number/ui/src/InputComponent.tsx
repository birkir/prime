...
export class InputComponent extends React.PureComponent<IPrimeFieldProps> {
  ...
  public render() {
    ...
    if (rules.min && rules.minValue) {
      ...
      fieldRules.push({
        validator: (_, value, cb) => cb(Number(value) >= min ? undefined : `${field.title} must be greater or equal to ${min}`)
      });
    }
    ...
    if (rules.max && rules.maxValue) {
      ...
      fieldRules.push({
        validator: (_, value, cb) => cb(Number(value) <= max ? undefined : `${field.title} must be less or equal to ${max}`)
      });
    }
