import { defaults, difference, union } from 'lodash';
import { Sequelize } from 'sequelize';

import { contract } from './contract';
import { clean, setup } from './dbTasks';
...
SequelizeBackend.prototype = {
   ...
   * @return {Promise}
   */
  async end(transactions: Array<() => void>, cb: () => void): Promise<any> {
    (contract(arguments) as any).params('object', 'function').end();

    await Promise.all(
      transactions.map((transactionFn: any) => {
  ...
  async get(bucket, key, cb) {
    (contract(arguments) as any).params('string', 'string|number', 'function').end();
   ...
   * @return {Promise}
   */
  union(bucket: string, keys: string[], cb: (a, b) => void): Promise<any> {
    (contract(arguments) as any).params('string', 'array', 'function').end();
  ...
  add(transaction, bucket, key, values) {
    (contract(arguments) as any).params('object', 'string', 'string|number', 'string|array|number').end();
  ...
  del(transaction, bucket, keys) {
    (contract(arguments) as any).params('object', 'string', 'string|array').end();
   ...
   * @return {Promise}
   */
  remove(transaction: Array<() => void>, bucket: string, key: string | number, values: Array<string | number>) {
    (contract(arguments) as any).params('object', 'string', 'string|number', 'string|array|number').end();
